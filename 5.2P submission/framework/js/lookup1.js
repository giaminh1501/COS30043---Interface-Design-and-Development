...
Vue.createApp({
	...
	data() {
		return {
			...
			lookUp: {code:'',desc:'',cp:'',type:''},   // record the user selection of student name and subject
    ...
    filterMarks: function(){
		return this.allMarks.filter(m=>
			m.code.toLowerCase().match(this.lookUp.code.toLowerCase()) &&
			m.desc.toLowerCase().match(this.lookUp.desc.toLowerCase()) &&
			m.type.toLowerCase().match(this.lookUp.type.toLowerCase())           
			);
		}
	}
}).mount('#app');
